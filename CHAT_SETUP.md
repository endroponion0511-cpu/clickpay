...
Чтобы не переполнять базу, можно периодически удалять старые диалоги:

1. Откройте **SQL Editor** в Supabase
2. Скопируйте и выполните содержимое файла `supabase-chat-cleanup.sql`

После этого в админке появится кнопка **Clean history**. Она удаляет:
- Закрытые диалоги старше 30 дней и их сообщения
- Заброшенные диалоги (открыли, получили автоответ, не ответили) старше 14 дней
